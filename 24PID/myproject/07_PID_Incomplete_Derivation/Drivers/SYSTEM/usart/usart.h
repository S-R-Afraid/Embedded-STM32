...
extern UART_HandleTypeDef g_uartx_handle[4];       /* HAL UART句柄 */

extern uint8_t  g_usart_rx_buf[USART_REC_LEN];  /* 接收缓冲,最大USART_REC_LEN个字节.末字节为换行符 */
extern uint16_t g_usart_rx_sta;                 /* 接收状态标记 */
extern uint8_t g_rx_buffer[RXBUFFERSIZE];       /* HAL库USART接收Buffer */

extern uint8_t g_uart2_rx_buffer;

void usart_init(uint8_t x,uint32_t baudrate);                   /* 串口初始化函数 */

int my_USART_printf(uint8_t x, char *format, ... );             /*指定串口发送*/

uint16_t GET_g_usart_rx_sta(void);

void Init_g_usart_rx_sta(void);
