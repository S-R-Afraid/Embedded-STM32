...
{
    BitValue = BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET;
	HAL_GPIO_WritePin(IIC_SCL_GPIO_PORT, IIC_SCL_GPIO_PIN, BitValue);		//根据BitValue，设置SCL引脚的电平
...
{
    BitValue = BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET;
	HAL_GPIO_WritePin(IIC_SDA_GPIO_PORT, IIC_SDA_GPIO_PIN, BitValue);		//根据BitValue，设置SDA引脚的电平，BitValue要实现非0即1的特性
	delay_us(10);												//延时10us，防止时序频率超过要求
}
...
{
	uint8_t BitValue;
	BitValue = HAL_GPIO_ReadPin(IIC_SDA_GPIO_PORT, IIC_SDA_GPIO_PIN);		//读取SDA电平
	delay_us(10);												//延时10us，防止时序频率超过要求
