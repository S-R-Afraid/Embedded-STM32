...
USART_TypeDef *printf_USART = USART_UX;
int fputc(int ch, FILE *f)
{
    //MY_USART_FPUTC_NUM++;
    while ((printf_USART->SR & 0X40) == 0);     /* 等待上一个字符发送完成 */
    //LED1_TOGGLE();
    printf_USART->DR = (uint8_t)ch;             /* 将要发送的字符 ch 写入到DR寄存器 */
    return ch;
}
#endif
/******************************************************************************************/
...
uint8_t g_uart2_rx_buffer;          /*串口2接收缓冲*/

UART_HandleTypeDef g_uartx_handle[4];       /* HAL UART句柄 */
USART_TypeDef* USARTx[4]={USART1,USART1,USART2,USART3 };
 ...
 * @retval      无
 */
void usart_init(uint8_t x,uint32_t baudrate)
{
    /*UART 初始化设置*/
    g_uartx_handle[x].Instance = USARTx[x];                                   /* USART_UX即串口1(宏定义里) */
    g_uartx_handle[x].Init.BaudRate = baudrate;                              /* 波特率 */
    g_uartx_handle[x].Init.WordLength = UART_WORDLENGTH_8B;                  /* 字长为8位数据格式 */
    g_uartx_handle[x].Init.StopBits = UART_STOPBITS_1;                       /* 一个停止位 */
    g_uartx_handle[x].Init.Parity = UART_PARITY_NONE;                        /* 无奇偶校验位 */
    g_uartx_handle[x].Init.HwFlowCtl = UART_HWCONTROL_NONE;                  /* 无硬件流控 */
    g_uartx_handle[x].Init.Mode = UART_MODE_TX_RX;                           /* 收发模式 */
    HAL_UART_Init(&g_uartx_handle[x]);                                       /* HAL_UART_Init()会使能UARTx */

    /* 该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量 */
    //HAL_UART_Receive_IT(&g_uartx_handle[x], (uint8_t *)g_rx_buffer, RXBUFFERSIZE); 
}
...
如果你觉得外设太多全写一个函数里不好，那就不用这个函数，定义自己的函数，或者把这些函数单独写在一个文件里*/
{
    ...
    if (huart->Instance == USART_UX)                            /* 如果是串口1，进行串口1 MSP初始化 */
    {
        ...
        HAL_UART_Receive_IT(&g_uartx_handle[1], (uint8_t *)g_rx_buffer, RXBUFFERSIZE); 
...

{
    ...
    if (huart->Instance == USART2){
        HAL_UART_Transmit(&g_uartx_handle[1],(uint8_t *)&g_uart2_rx_buffer, 1, HAL_MAX_DELAY);
        //my_USART_printf(1,(char *)&g_uart2_rx_buffer);  //将接收到的数据从串口一发送回去
...
/*指定串口发送格式化字符串，x为指定的串口号，如USART1，x=1*/
int my_USART_printf(uint8_t x, char *format, ... ){
    printf_USART = USARTx[x];
    va_list args;
    //va_list args_copy;
