...
还有一种情况就是当两个或者多个中断的抢占式优先级和响应优先级相同时，
    那么就遵循自然优先级，看中断向量表的中断排序，数值越小，优先级越高。

外部中断调用流程(以EXTI4为例)：
    
    发生中断->调用外部中断服务函数EXTI4_IRQHandler()
                            ⇲>调用公共处理函数HAL_GPIO_EXTI_IRQHandler清除中断位
                                    ⇲>调用回调函数HAL_GPIO_EXTI_Callback执行公共操作
*/
...
void KEY0_INT_IRQHandler(void)/*这些外部中断程序的函数名经过预编译处理后会转换成对应的中断线处理程序（向量表里的weak函数）
